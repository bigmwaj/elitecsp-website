...
- **Slug generation**: derived from the French title when available
  (otherwise English), lowercased, accent-stripped, hyphenated. Slugs must
  stay stable across repeated runs for the same job.
- **`jobId` upsert rule**: match existing jobs by slug; update in place if
  found, otherwise insert with `jobId = max(existing jobId) + 1`. `jobId`
